...
class Bag(object):
    ...
    def validate(self, processes=1, fast=False, complete=False):
        ...
        self._validate_contents(processes=processes, fast=fast, complete=complete)
        return True
    ...
    def _validate_contents(self, processes=1, fast=False, complete=False):
        if complete:
            self._validate_complete() # Pretty fast
        if fast:
            if not self.has_oxum():
                raise BagValidationError("cannot validate Bag with fast=True if Bag lacks a Payload-Oxum")
            else:
                self._validate_oxum()    # Fast
        if not fast and not complete:
            self._validate_oxum()
    ...
    for bag_dir in args.directory:
        ...
        if args.validate:
            try:
                ...
                bag.validate(processes=args.processes, fast=args.fast, complete=args.complete)
                if args.fast:
                    LOGGER.info("%s valid according to Payload-Oxum", bag_dir)
                if args.complete:
                    LOGGER.info("%s complete", bag_dir)
                else:
